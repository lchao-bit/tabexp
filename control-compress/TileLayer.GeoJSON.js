    ...
    _xhrHandler: function (req, layer, tile, tilePoint) {
        return function () {
            ...
            if ((s >= 200 && s < 300 && s != 204) || s === 304) {
				var input = new Uint8Array(req.response);
				var output=bro.decompressArray(input);
				var str = arrayBufferToString(output);
				
				//var output=decompfunc(req.responseText);
                tile.datum = JSON.parse(str);
                //tile.datum = JSON.parse(req.responseText);
				
				//decrypt(tile.datum.coordinates);
